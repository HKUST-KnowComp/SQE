...
def sample_58_test_data(id):
    n_queries_valid_test_dict = n_queries_valid_test_dict_larger
    all_query_types = pd.read_csv("../preprocess/test_generated_formula_anchor_node=3_filtered.csv").reset_index(
        drop=True)  # debug
    ...
    for data_dir in n_queries_valid_test_dict.keys():
        ...
        with open("../sampled_data_58_eval/" + data_dir + "_valid_queries_" + str(id) + ".json", "w") as file_handle:
        ...
        with open("../sampled_data_58_eval/" + data_dir + "_test_queries_" + str(id) + ".json", "w") as file_handle:
            json.dump(test_queries, file_handle)


if __name__ == "__main__":
    with Pool(num_processes) as p:
        print(p.map(sample_58_test_data, range(num_processes)))
