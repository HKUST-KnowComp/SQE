...
class TreeRNN(model.IterativeModel):
    def __init__(self, num_entities, num_relations, embedding_size, label_smoothing=0.1, use_old_loss=False,negative_size=128):
        super(TreeRNN, self).__init__(num_entities, num_relations, embedding_size, use_old_loss)
    ...
    def projection(self, relation_ids, sub_query_encoding):
        ...
        next_h = self.tanh(self.W_ih(x) + self.W_hh(prev_h))




        return next_h#torch.stack((next_h, next_c), dim=1)
    ...
    def intersection(self, sub_query_encoding_list):
        ...
        # [batch_size, embedding_size]
        prev_h = all_subquery_encodings[:, :, :].sum(dim=1)
...
if __name__ == "__main__":
    ...
    model = TreeRNN(num_entities=nentity, num_relations=nrelation, embedding_size=300)
