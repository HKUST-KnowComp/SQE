...
class TransformerModel(SequentialModel):
    def __init__(self, num_entities, num_relations, embedding_size, label_smoothing=0.1, num_layers=3):
        super(TransformerModel, self).__init__(num_entities, num_relations, embedding_size)

        config = BertConfig(hidden_size=embedding_size, vocab_size=num_entities + num_relations + std_offset,
                            num_attention_heads=16, num_hidden_layers=num_layers, position_embedding_type='relative_key')#for study on improving compositional generalizability
    ...
    def encode(self, batched_structured_query):
        ...
        encoded = self.transformer_encoder(batched_structured_query).pooler_output
...
if __name__ == "__main__":
    ...
    model = TransformerModel(num_entities=nentity, num_relations=nrelation, embedding_size=256)
