...
class GRUModel(SequentialModel):
    def __init__(self, num_entities, num_relations, embedding_size, label_smoothing=0.1, num_layers=3):
        super(GRUModel, self).__init__(num_entities, num_relations, embedding_size)
        ...
        self.GRU = nn.GRU(input_size=embedding_size, hidden_size=embedding_size // 2, num_layers=num_layers,
                            batch_first=True,
                            bidirectional=True)
...
if __name__ == "__main__":
    ...
    model = GRUModel(num_entities=nentity, num_relations=nrelation, embedding_size=300)
